...
async function openSupportModal(sessionToken, course) {
  ...
  try {
    ...
    body.innerHTML = items.length ? items.map((item) => `<a class="support-link-button" href="${escapeHtml(item.url)}" target="_blank" rel="noreferrer"><span>${escapeHtml(item.label || item.title || item.name || item.item || '문의')}</span><span class="support-link-meta">오픈카톡으로 이동</span></a>`).join('') : '<div class="empty-state">등록된 고객센터가 없습니다.</div>';
  } catch (err) {
